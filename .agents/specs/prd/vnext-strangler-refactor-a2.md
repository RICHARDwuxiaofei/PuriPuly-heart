...
status: reviewed
source: .agents/specs/prd/drafts/vnext-strangler-refactor-a2.source.r3.md
...
PuriPuly Heart reaches a maintainable owner-based backend and a Flet 0.85 UI without sacrificing the current vnext product: every production responsibility is transferred incrementally behind the working UI, every transfer preserves verified Windows behavior, and the final fixed-size UI preserves the verified baseline appearance and interaction exactly at 1136 by 850 before the legacy UI, `GuiController`, and multi-responsibility `ClientHub` ownership are retired.
...
- Existing settings files and keys remain loadable with the same meaning, forward migration backs up before rewrite, persistence remains round-trippable, and defaults remain safe.
...
### NG-003 — Product redesign

Changing information architecture, visual design, copy, navigation, workflows, defaults, features, provider behavior, or output semantics is not authorized.
...
Owner replacement and UI migration must preserve existing settings and persistence meaning, backup and migration safety, SecretStore compatibility, provider aliases, prompt fallback behavior, Broker `/v1`, overlay protocols and startup behavior, executable and installer identity, locale availability, and established user-data locations.
...
The replacement UI must be visually and interactively identical to the verified baseline UI at the established 1136 by 850 main-window size, not merely similar or functionally equivalent. The main window must remain fixed at 1136 by 850 and must not permit user resizing. At that size it must preserve the window shell, title bar, navigation, screen composition, control types, ordering, positioning, visibility, enabled and selected states, labels, localized text, icons, colors, typography, spacing, sizing, scrolling, focus behavior, modal stacking and dismissal, loading and error feedback, and established interaction behavior.

Parity applies to the main views, all settings surfaces, all established dialogs and modals, and the baseline states reachable in normal and explicitly gated debug-preview operation across every supported locale at 1136 by 850. Minimum-window-size behavior, resize-responsive behavior, and rendering or interaction at any other main-window dimension are outside this contract. Any user-perceptible difference at the fixed size is a failed acceptance result unless the product owner explicitly approves it through a reviewed PRD revision.
...
The verified baseline UI appearance and interaction at the fixed 1136 by 850 main-window size are the authoritative replacement target. No redesign or unapproved visible deviation is permitted at that size.
...
Valid existing settings and credentials remain usable with the same meaning, settings remain round-trippable, and forward migration remains backed up and failure-safe.
...
- The Flet 0.85 UI must preserve the verified baseline appearance exactly; UI redesign is not authorized.
...
| AC-006 | REQ-006, INV-P-005, INV-P-006 | automated compatibility + production-composition + restart + migration | Historical supported settings fixtures, SecretStore test environments, configured providers, supported Windows x64, and Linux-native Broker workspace only if Broker changes | Settings round-trip and supported forward migration preserve meaning and backups; credentials remain available through compatible SecretStore keys; provider aliases and prompts resolve as before; restart restores the same state; any touched Broker or provider compatibility checks pass in their required environment. |
...
| AC-008 | REQ-008, INV-P-003 | comparative visual + comparative interaction + automated window-constraint inspection + manual product-owner review | Baseline Flet 0.28.3 and replacement Flet 0.85 on identical supported Windows x64 environment, theme, display scale, locale, settings, data, window state, and content; main window fixed at 1136x850 | Side-by-side and overlaid captures plus direct interaction comparison at 1136x850 cover the shell, every main view, every settings surface, every established dialog/modal, normal states, error/loading/disabled/selected states, gated debug-preview states, and all five locale bundles; inspection and direct manipulation prove the replacement main window remains 1136x850 and cannot be user-resized; there is no user-perceptible visual or interaction difference at that size, and any discrepancy is a failure unless explicitly approved in a reviewed PRD revision. |
