...
CREATE TABLE openrouter_entitlements_delivery_ack_migration (
  ...
  CHECK (
    ...
    OR (verified_hardware_hash IS NOT NULL AND verified_hardware_hash_salt_version IS NOT NULL)
  ),
  CHECK (installation_id = lower(installation_id)),
  CHECK (length(installation_id) BETWEEN 1 AND 128),
  CHECK (verified_hardware_hash IS NULL OR length(verified_hardware_hash) BETWEEN 1 AND 128)
) STRICT;

INSERT INTO openrouter_entitlements_delivery_ack_migration (
  installation_id,
  status,
  budget_usd,
...
ALTER TABLE openrouter_entitlements_delivery_ack_migration RENAME TO openrouter_entitlements;
...
CREATE INDEX idx_openrouter_entitlements_hardware_hash_current
  ON openrouter_entitlements(verified_hardware_hash, verified_hardware_hash_salt_version)
  WHERE verified_hardware_hash IS NOT NULL;
...
CREATE TABLE qq_managed_entitlements_delivery_ack_migration (
...
INSERT INTO qq_managed_entitlements_delivery_ack_migration (
  qq_subject_ref,
  status,
  issue_ref,
  managed_credential_ref,
...
ALTER TABLE qq_managed_entitlements_delivery_ack_migration RENAME TO qq_managed_entitlements;
