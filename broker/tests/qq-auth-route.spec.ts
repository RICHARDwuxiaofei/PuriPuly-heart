...
describe('QQ auth assertion route', () => {
  ...
  it('verifies a valid QQ assertion and stores only derived values', async () => {
  ...
  it('returns already_verified for a repeated valid assertion without mutating the original row', async () => {
    const env = createTestBrokerEnv();
  ...
  it('fails closed when QQ_AUTH_HMAC_PSK is blank', async () => {
    const env = createTestBrokerEnv();
    env.QQ_AUTH_HMAC_PSK = '   ';
    const qqIdentity = 'qq-openid-blank-secret-user';
