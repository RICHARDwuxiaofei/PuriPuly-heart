...
Scope: vNext telemetry broker port and managed-key delivery ACK rollout readiness. This note records redacted verification evidence and unresolved external-environment blockers. It does not claim production deployment or real-user smoke completion.
...
Automated tests provide partial synthetic coverage for the ACK behavior, but they do not replace the source-listed real external smoke scenarios:
...
Real external smoke tests are not complete. The source-listed Discord, QQ, OpenRouter usage, app-exit, and response-loss smoke scenarios require external accounts, a production-like broker/D1 environment, OpenRouter usage that mutates provider state, and explicit evidence-handling authorization. Running them without that setup risks mutating production state or exposing operational secrets in evidence.

Required real-smoke blockers before release readiness can be claimed:
...
Automated vNext broker and Python evidence passed, local migration ordering is correct, remote D1 has no unapplied migrations, and rollout order remains broker-first/client-after-broker. The rollout-readiness gate remains blocked on real redacted Discord/QQ/OpenRouter smoke evidence.
