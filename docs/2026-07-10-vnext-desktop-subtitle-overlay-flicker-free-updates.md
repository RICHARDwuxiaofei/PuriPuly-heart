...
- The diagnostic subcommands are test/diagnostic surfaces only. They are unavailable through normal product UI, require explicit invocation, do not load or persist settings, do not load secrets, and make no provider, broker, output-routing, or external-network calls.
...
| Width-history burst | `12, 13, 14` | Final short fixture at revision `14`, retaining the long fixture's width floor from `13` | `12=superseded`, `13=superseded`, `14=committed` |
...
One cycle therefore has exactly `9` committed, `3` superseded, `1` stale, and `0` failed outcomes. The default 100-cycle run must report totals of `900` committed, `300` superseded, `100` stale, and `0` failed outcomes. Any different disposition or missing render-commit acknowledgement is a failed run.
...
- Repro-harness tests must execute every normative batch, assert its exact FIFO dispositions and default 100-cycle totals, and fail when a disposition, acknowledgement, or final visual state differs from the schedule.
...
- Manual Windows QA is required because automated Flet doubles cannot prove real transparent-window compositor output. The required installed-application route is `PuriPulyHeart.exe run-desktop-overlay-repro --cycles 100 --dwell-ms 150 --output-dir <empty-output-directory>`, followed by `PuriPulyHeart.exe verify-desktop-overlay-repro --output-dir <empty-output-directory>`. The exact source-development equivalent is `.venv\Scripts\python.exe -m puripuly_heart.main run-desktop-overlay-repro --cycles 100 --dwell-ms 150 --output-dir <empty-output-directory>`, followed by the analogous `verify-desktop-overlay-repro` command. The rebuilding `--preview` route is not valid evidence.
