...
use puripuly_heart_overlay::{
    ...
    OverlayResubmitCurrentFrame, OverlayRuntime, RenderedFrame, ResubmitCurrentFrameOutcome,
...
fn snapshot_with_blocks(
    ...
    snapshot_with_calibration(revision, OverlayPresentationCalibration::default(), blocks)
}

fn snapshot_with_calibration(
    revision: u64,
...
fn text_snapshot(revision: u64) -> OverlayPresentationSnapshot {
    snapshot_with_blocks(revision, vec![block("self:1", "self", "hello", "", true)])
}
...
struct RecordingSubmitter {
    ...
    applied_calibrations: Vec<OverlayPresentationCalibration>,
    submitted_texture_ids: Vec<usize>,
}
...
impl OverlayFrameSubmitter for RecordingSubmitter {
    fn submit_frame(&mut self, frame: &RenderedFrame) -> Result<(), OpenVrError> {
        ...
        self.submitted_texture_ids
            .push(frame.texture_ptr().unwrap_or(std::ptr::null_mut()) as usize);
        if self.fail {
    ...
    fn apply_calibration(
        ...
        self.applied_calibrations.push(calibration.clone());
        Ok(())
    }
}
...
#[tokio::test]
async fn runtime_resubmit_noops_without_stored_frame() {
    let logger = test_logger("resubmit-no-stored-frame").await;
    let mut runtime = OverlayRuntime::new(OverlayPresentationSnapshot::default());
    let mut submitter = RecordingSubmitter::default();
...
#[tokio::test]
async fn runtime_resubmit_noops_until_target_revision_has_been_submitted() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-target-not-ready").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(text_snapshot(3));
    let mut submitter = RecordingSubmitter::default();
...
#[tokio::test]
async fn runtime_resubmit_submits_stored_text_frame_without_redraw() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-stored-text").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(text_snapshot(8));
    let mut submitter = RecordingSubmitter::default();
    ...
    assert_eq!(runtime.last_submitted_revision(), Some(8));
    ...
    assert_eq!(
        ...
        vec!["submit:text", "show", "submit:text"]
    );
    assert_eq!(runtime.state().snapshot().revision, 8);
    assert!(!runtime.redraw_requested());
    assert_eq!(runtime.clone(), runtime);
}

#[tokio::test]
async fn runtime_resubmit_shows_hidden_overlay_after_submitting_stored_text_frame() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-hidden-overlay-order").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(text_snapshot(8));
    let mut submitter = RecordingSubmitter::default();
    ...
    assert_eq!(submitter.calls, 2);
    assert_eq!(
        &submitter.operations[submitter.operations.len() - 2..],
        &["submit:text", "show"]
    );
    assert_eq!(runtime.state().snapshot().revision, 8);
    assert!(!runtime.redraw_requested());
...
#[tokio::test]
async fn runtime_resubmit_applies_current_calibration_and_preserves_redraw_request() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-current-calibration").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(text_snapshot(8));
    let mut submitter = RecordingSubmitter::default();
    ...
    runtime
        ...
        .await
        .unwrap();
    let current_calibration = OverlayPresentationCalibration {
        anchor: "head_locked".into(),
        offset_x: 0.2,
        offset_y: -0.15,
        distance: 1.6,
    ...
    runtime.apply_snapshot(snapshot_with_calibration(
        9,
        current_calibration.clone(),
        vec![block("self:1", "self", "hello", "", true)],
    ));
    ...
    assert_eq!(outcome, ResubmitCurrentFrameOutcome::Submitted);
    assert_eq!(submitter.applied_calibrations.len(), 2);
    assert_eq!(
        submitter.applied_calibrations.last(),
        Some(&current_calibration)
    );
    assert_eq!(runtime.state().snapshot().revision, 9);
    assert!(runtime.redraw_requested());
}

#[tokio::test]
async fn runtime_resubmit_noop_preserves_existing_redraw_request() {
    let logger = test_logger("resubmit-noop-preserves-redraw").await;
    let mut runtime = OverlayRuntime::new(OverlayPresentationSnapshot::default());
    let mut submitter = RecordingSubmitter::default();

    runtime.apply_snapshot(text_snapshot(1));
    assert!(runtime.redraw_requested());
...
#[tokio::test]
async fn runtime_resubmit_noops_for_empty_stored_frame() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-empty-stored-frame").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(OverlayPresentationSnapshot::default());
    let mut submitter = RecordingSubmitter::default();
...
#[tokio::test]
async fn runtime_resubmit_noops_when_current_state_has_no_drawable_text() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-current-empty").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(text_snapshot(8));
    let mut submitter = RecordingSubmitter::default();
    ...
    runtime
        ...
        .await
        .unwrap();
    runtime.apply_snapshot(snapshot_with_blocks(9, vec![]));
...
#[tokio::test]
async fn runtime_does_not_advance_stored_frame_when_normal_submit_fails() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-normal-submit-fails").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(text_snapshot(9));
    ...
    assert_eq!(runtime.last_submitted_revision(), None);
}

#[tokio::test]
async fn runtime_resubmit_failure_uses_openvr_failure_path() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-submit-fails").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(text_snapshot(10));
    let mut submitter = RecordingSubmitter::default();
...
#[tokio::test]
async fn runtime_resubmit_keeps_last_successful_texture_after_failed_later_submit() {
    let renderer = CaptionRenderer::new_for_test().unwrap();
    let logger = test_logger("resubmit-keeps-successful-texture").await;
    let (mut bridge, server) = connect_test_bridge().await;
    let mut runtime = OverlayRuntime::new(snapshot_with_blocks(
        1,
        vec![block("self:1", "self", "first text", "", true)],
    ));
    let mut submitter = RecordingSubmitter::default();
    ...
    runtime
        ...
        .await
        .unwrap();
    let successful_texture_id = submitter.submitted_texture_ids[0];

    assert_eq!(runtime.last_submitted_revision(), Some(1));

    runtime.apply_snapshot(snapshot_with_blocks(
        2,
        vec![block("self:1", "self", "second text", "", true)],
    ));
    submitter.fail = true;
    ...
    let err = runtime
        ...
        .await
        .unwrap_err();
    let failed_texture_id = submitter.submitted_texture_ids[1];

    assert!(matches!(err, RuntimeFailure::OpenVr(_)));
    assert_eq!(runtime.last_submitted_revision(), Some(1));
    assert_ne!(failed_texture_id, successful_texture_id);
    ...
    let outcome = runtime
        ...
        .await
        .unwrap();
    let resubmitted_texture_id = submitter.submitted_texture_ids[2];

    drop(bridge);
    server.await.unwrap();

    assert_eq!(outcome, ResubmitCurrentFrameOutcome::Submitted);
    assert_eq!(resubmitted_texture_id, successful_texture_id);
    assert_ne!(resubmitted_texture_id, failed_texture_id);
    assert_eq!(runtime.state().snapshot().revision, 2);
    assert_eq!(runtime.last_submitted_revision(), Some(1));
}

#[tokio::test]
async fn runtime_event_loop_handles_resubmit_current_frame_without_redraw() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let server = tokio::spawn(async move {
        let (stream, _) = listener.accept().await.unwrap();
    ...
    let logger = test_logger("event-loop-resubmit").await;
    let mut manifest = test_manifest();
    manifest.bridge_url = format!("ws://{}", address);
...
#[tokio::test]
async fn bridge_client_receives_resubmit_current_frame_message() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let server = tokio::spawn(async move {
        let (stream, _) = listener.accept().await.unwrap();
    ...
    assert!(matches!(
        message,
        puripuly_heart_overlay::BridgeIncoming::ResubmitCurrentFrame(request)
            if request.target_revision == 7
                && request.reason.as_deref() == Some("peer_presentation_refresh")
    ));
    server.await.unwrap();
}

#[tokio::test]
async fn bridge_client_rejects_resubmit_current_frame_without_target_revision() {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let server = tokio::spawn(async move {
        let (stream, _) = listener.accept().await.unwrap();
    ...
    assert!(
        matches!(error, puripuly_heart_overlay::BridgeError::Protocol(message) if message.contains("target_revision"))
    );
    server.await.unwrap();
}
