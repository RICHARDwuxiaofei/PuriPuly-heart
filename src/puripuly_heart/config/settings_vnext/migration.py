...
def _migrate_multi_model_gemma_translation(translation: dict[str, Any]) -> None:
    ...
    if (
        ...
        and migrated_primary_gemma
        and fallback.get("model") == "deepseek_v4_flash"
        and fallback.get("connection") in {"official_byok", "openrouter"}
    ):
        fallback["model"] = "gemma4_26b_31b"
        fallback["connection"] = connection
        fallback["selection_alias"] = (
            "managed_gemma4_26b_31b" if connection == "managed" else "openrouter_gemma4_26b_31b"
        )
    elif isinstance(fallback, dict) and fallback.get("model") == "gemma4":
