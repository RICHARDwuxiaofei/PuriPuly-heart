...
class OverlayProcessManager:
    ...
    async def _wait_for_startup(self) -> None:
        ...
        try:
            while True:
                    ...
                    {event_task, exit_task, timeout_task},
                ...
                if event_task in done:
                    ...
                    if event_type == "overlay_ready":
                        self.state = "connected"
                        self.failure_reason = None
                        self._monitor_task = asyncio.create_task(self._monitor_process_exit())
            ...
            for task in (event_task, exit_task, timeout_task):
                if not task.done():
                    task.cancel()
            await asyncio.gather(event_task, exit_task, timeout_task, return_exceptions=True)
    ...
    async def _monitor_process_exit(self) -> None:
        ...
        if self.state == "connected" and exit_code is not None:
            await self._fail("runtime_crashed", cleanup_manifest=False)
    ...
    async def _fail(self, failure_reason: str, *, cleanup_manifest: bool = True) -> None:
        self.state = "failed"
