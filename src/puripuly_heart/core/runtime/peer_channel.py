...
class PeerChannelRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        diagnostic_sink: Callable[[PeerRuntimeDiagnostic], Awaitable[None] | object] | None = None,
    ) -> None:
        self.hub = hub
        self.clock = clock
    ...
    def bind_diagnostic_sink(
        self,
        sink: Callable[[PeerRuntimeDiagnostic], Awaitable[None] | object] | None,
    ) -> None:
    ...
    async def apply_policy(
        ...
        *,
        ...
        if desired_active:
            ...
            if (
                release_dormant
                and getattr(config.backend, "provider", None) == "local_qwen"
                and (lease := self._provider_read_port.lease_stt_provider("peer_stt")) is not None
                and lease.current is not None
            ):
                await self.hub.drain_peer_stt_for_toggle_off(
                    lease.current,
    ...
    def _diagnostic_for_exception(
        ...
        if isinstance(exc, (ProcessAudioCaptureSetupError, ProcessAudioCaptureUnavailableError)):
            return PeerRuntimeDiagnostic(
                PeerRuntimeFailureReason.PROCESS_SETUP_FAILED, capture_kind
            )
        if capture_kind == "process":
            return PeerRuntimeDiagnostic(
                PeerRuntimeFailureReason.PROCESS_SOURCE_FAILED, capture_kind
            )
        return PeerRuntimeDiagnostic(PeerRuntimeFailureReason.PEER_RUNTIME_FAILED, capture_kind)

    async def _publish_failure(self, diagnostic: PeerRuntimeDiagnostic) -> None:
