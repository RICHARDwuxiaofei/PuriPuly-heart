...
class Presenter:
    ...
    def __init__(self) -> None:
        self.bridge = None
        self.closed = 0
        self.retry_ownership: list[bool] = []
    ...
    async def update_native_retry_ownership(self, confirmed: bool) -> None:
        self.retry_ownership.append(confirmed)
...
class Factories:
    def __init__(
        ...
        self.retry_ownership_changed = None
    ...
    def create_process_manager(self, **values: object) -> Manager:
        self.retry_ownership_changed = values["retry_ownership_changed"]
        return self.manager
...
@pytest.mark.asyncio
async def test_process_manager_retry_ownership_routes_to_presenter() -> None:
    factories = Factories()
    runtime = OverlayOscApplicationRuntime(ingress=Ingress(), factories=factories)
    await runtime.start_overlay(configuration())
    await settle()
    assert factories.retry_ownership_changed is not None
    await factories.retry_ownership_changed(True)
    assert factories.presenters[0].retry_ownership == [True]
