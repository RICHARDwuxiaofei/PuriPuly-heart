...
class Hub:
    def __init__(self, events: list[str]) -> None:
        ...
        self.stt_providers = {"self_stt": None, "peer_stt": None}
    ...
    def lease_stt_provider(self, slot):  # noqa: ANN001, ANN201
        provider = self.stt_providers[slot]
        if provider is None:
            return None
        return SimpleNamespace(current=provider)
...
async def test_local_qwen_reenable_reuses_retained_production_provider_before_idle_expiry() -> None:
    ...
    retained = object()
    hub.stt_providers["self_stt"] = retained

    await host.execute(SetSelfSTTEnabled(True))
    await host.execute(SetSelfSTTEnabled(False))
    await host.execute(SetSelfSTTEnabled(True))

    assert not any(event.startswith("providers_install:") for event in events)
    assert events == ["self:True", "self:False", "self:True"]


@pytest.mark.asyncio
async def test_peer_resume_reinstalls_missing_provider_after_idle_expiry() -> None:
    events: list[str] = []
    receipt = SettingsCommitReceipt(AppSettingsVNext(), "peer-expired", "toggle", None)
    host, _hub, _peer, _transactions = _host(events, receipt)

    result = await host.resume_peer_stt()

    assert result.status == RUNTIME_APPLY_STATUS_APPLIED
    assert events == [
        "providers_install:peer-expired:peer_stt",
        "peer:resume:peer-expired",
    ]
