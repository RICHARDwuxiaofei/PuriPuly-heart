...
def test_load_prompt_for_openrouter_matches_file() -> None:
    prompt = load_prompt_for_provider("openrouter")
    raw = Path("prompts/openrouter.md").read_text(encoding="utf-8").strip()
    assert prompt == raw
    assert prompt
