...
@pytest.mark.asyncio
async def test_provider_claim_returns_identity_delta_without_mutating_state_or_secrets() -> None:
    harness = FakeDiscordOAuthHarness()
    client = FakeManagedReleaseClient(
        discord_start_result=_make_discord_start_success(redirect_uri=harness.redirect_uri),
        discord_issue_result=ManagedOpenRouterIssueSuccess(
            openrouter_api_key="managed-key",
            managed_credential_ref="credential-ref",
        ),
    )
    ...
    before_state = asdict(settings.managed_identity)
    before_secrets = dict(secrets._items)

    result = await service.claim_discord_managed_key(
        referral_id="referral",
    )

    assert result.status == "claimed"
    assert result.managed_credential_ref == "credential-ref"
    assert result.identity is not None
    assert result.identity.generated is True
    assert result.identity.secret_values
    assert asdict(settings.managed_identity) == before_state
    assert secrets._items == before_secrets
    assert "managed-key" not in repr(result)


@pytest.mark.asyncio
async def test_production_bound_release_token_issue_routes_only_through_transaction_port() -> None:
    client = FakeManagedReleaseClient()
    service, _settings, _secrets = _make_service(client=client)

    class Port:
        calls = 0

        async def ensure_key_for_llm_start(
            self, release, *, available_api_key
        ):  # noqa: ANN001, ANN201
            self.calls += 1
            assert release is service
            assert available_api_key is None
            return ManagedOpenRouterReleaseResult(
                ManagedOpenRouterReleaseBehavior.READY,
                "managed_release.ready",
                api_key="coordinator-key",
                local_key_available=True,
            )

    port = Port()
    service.bind_transaction_port(port)

    result = await service._await_or_start_issue_flow()

    assert result.behavior is ManagedOpenRouterReleaseBehavior.READY
    assert result.api_key == "coordinator-key"
    assert port.calls == 1
    assert client.calls == []


@pytest.mark.asyncio
async def test_production_bound_existing_key_and_pending_ack_route_through_transaction_port() -> (
    None
):
    client = FakeManagedReleaseClient()
    ...
    settings.managed_identity.pending_delivery_ack_delivery_id = "delivery"
    settings.managed_identity.pending_delivery_ack_managed_credential_ref = "credential"
    ...
    class Port:
        ...
        async def ensure_key_for_llm_start(
            ...
            self.calls += 1
            assert release is service
            assert available_api_key == "managed-key"
            return ManagedOpenRouterReleaseResult(
                ManagedOpenRouterReleaseBehavior.READY,
                "managed_release.ready",
...
@pytest.mark.asyncio
async def test_production_bound_prepare_does_not_run_legacy_claim_flow() -> None:
    client = FakeManagedReleaseClient()
    service, _settings, _secrets = _make_service(client=client)
...
@pytest.mark.parametrize(
    ("error", "behavior", "message_key", "retry_after_ms", "clear_state"),
    [
        (
            ManagedOpenRouterReleaseError(
                "challenge_expired",
...
async def test_release_token_claim_preserves_inherited_failure_semantics(
    error,
    behavior,
    message_key,
    retry_after_ms,
    ...
    settings.managed_identity.release_token = "release-token"
    settings.managed_identity.release_token_expires_at = "2099-01-01T00:00:00Z"
    _set_verified_snapshot(settings)
    service, _settings, _secrets = _make_service(
        client=FakeManagedReleaseClient(issue_result=error),
        settings=settings,
        secrets=secrets,
    )

    result = await service.claim_release_token_managed_key()

    assert result.status == "failed"
    assert result.release_result is not None
    assert result.release_result.behavior is behavior
    assert result.release_result.message_key == message_key
    assert result.release_result.retry_after_ms == retry_after_ms
    assert result.clear_temporary_state is clear_state
    assert settings.managed_identity.release_token == "release-token"


@pytest.mark.asyncio
async def test_release_token_claim_distinguishes_invalid_verification_without_mutation() -> None:
    settings = AppSettings()
    secrets = InMemorySecretStore()
    ensure_managed_identity_bundle(
        ManagedIdentityStateAdapter(settings, lambda _updated: None),
    ...
    assert result.detail_code == "managed_verification_invalid"
    assert result.release_result.behavior is ManagedOpenRouterReleaseBehavior.RESTART
    assert result.clear_temporary_state is True
    assert settings.managed_identity.release_token == "release-token"
...
@pytest.mark.parametrize(
    "subcode",
    ["device_public_key_registered", "installation_binding_mismatch"],
)
async def test_release_token_binding_mismatch_prepares_replacement_without_mutation(
    subcode: str,
) -> None:
    settings = AppSettings()
    secrets = InMemorySecretStore()
    original = ensure_managed_identity_bundle(
        ManagedIdentityStateAdapter(settings, lambda _updated: None),
        secrets,
    )
    original_secrets = dict(secrets._items)
    settings.managed_identity.release_token = "release-token"
    _set_verified_snapshot(settings)
    service, _settings, _secrets = _make_service(
        client=FakeManagedReleaseClient(
    ...
    assert result.identity is not None
    assert result.identity.generated is True
    assert result.identity.bundle.installation_id != original.installation_id
    assert settings.managed_identity.installation_id == original.installation_id
    assert secrets._items == original_secrets
...
async def test_discord_binding_mismatch_prepares_replacement_without_mutation(
    subcode: str,
) -> None:
    harness = FakeDiscordOAuthHarness()
    service, settings, secrets, _client, _harness = _make_discord_service(
    ...
    original_state = asdict(settings.managed_identity)
    original_secrets = dict(secrets._items)

    result = await service.claim_discord_managed_key(referral_id=None)

    assert result.release_result.behavior is ManagedOpenRouterReleaseBehavior.RESTART
    assert result.identity is not None
    assert result.identity.generated is True
    assert asdict(settings.managed_identity) == original_state
    assert secrets._items == original_secrets


@pytest.mark.asyncio
async def test_bound_release_transaction_honors_recorded_retry_after_window() -> None:
    service, settings, _secrets = _make_service(client=FakeManagedReleaseClient())
    settings.managed_identity.release_token = "release-token"

    class Port:
        calls = 0

        async def ensure_key_for_llm_start(self, *_args, **_kwargs):  # noqa: ANN002, ANN003, ANN201
            self.calls += 1
            raise AssertionError("retry window must stop a new transaction")

    port = Port()
    service.bind_transaction_port(port)
    service.record_transaction_outcome(
        ManagedOpenRouterReleaseResult(
            ManagedOpenRouterReleaseBehavior.RETRY,
            "managed_release.retry_after_ms",
            retry_after_ms=9000,
    ...
    assert result.behavior is ManagedOpenRouterReleaseBehavior.RETRY
    assert result.message_key == "managed_release.retry_after_ms"
    assert result.retry_after_ms == 9000
    assert port.calls == 0


@pytest.mark.asyncio
async def test_bound_brake_outcome_does_not_create_retry_deadline() -> None:
    service, settings, _secrets = _make_service(client=FakeManagedReleaseClient())
    settings.managed_identity.release_token = "release-token"
    ...
    assert first.message_key == second.message_key == "managed_release.brake"
    assert port.calls == 2
...
@pytest.mark.asyncio
async def test_llm_start_defers_pending_delivery_ack_to_coordinator() -> None:
    settings = AppSettings()
    settings.openrouter.selected_source = OpenRouterCredentialSource.MANAGED
    ...
    assert result.diagnostics.code == "managed_delivery_ack_requires_coordinator"
    assert client.calls == []
