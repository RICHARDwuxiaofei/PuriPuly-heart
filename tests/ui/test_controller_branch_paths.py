...
@pytest.mark.asyncio
async def test_desktop_initial_control_manifest_routing_installs_desktop_runner_and_queues_startup_controls(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_overlay_runtime(monkeypatch)
...
async def test_desktop_reset_clears_position_unlocks_preserves_size_and_alpha_and_centers(
    ...
    assert {
        ...
        "mode": "edit",
    } in bridge.desktop_runtime_control_payloads
    assert bridge.desktop_runtime_control_payloads[-1] == {
        "command": "apply_window_bounds",
        "x": pytest.approx(160),
        "y": pytest.approx(333.5),
        "width": 1600,
        "height": 413,
        "bounds_epoch": 1,
    }
...
@pytest.mark.asyncio
async def test_desktop_reset_supersedes_late_stale_user_move_but_allows_future_move(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_overlay_runtime(monkeypatch)
    saved_desktop: list[tuple[object, object, str, bool, float]] = []
    ...
    await _wait_until(lambda: len(saved_desktop) == 1, attempts=20, delay_s=0.02)
    reset_bounds_payload = bridge.desktop_runtime_control_payloads[-1]
    reset_bounds_epoch = reset_bounds_payload.get("bounds_epoch")
    assert isinstance(reset_bounds_epoch, int)
    ...
    await renderer_events.put(
        {
            ...
            "payload": {
                ...
                "width": 1600,
                "height": 413,
                "bounds_epoch": reset_bounds_epoch - 1,
            },
        }
    )
    await asyncio.sleep(controller_module.DESKTOP_BOUNDS_PERSIST_DEBOUNCE_S * 2)
    ...
    await renderer_events.put(
        {
            ...
            "payload": {
                ...
                "width": 1600,
                "height": 413,
                "bounds_epoch": reset_bounds_epoch,
            },
        }
    )
    await _wait_until(lambda: len(saved_desktop) == 2, attempts=20, delay_s=0.02)

    assert saved_desktop[-1] == (500, 600, "large", False, 0.44)
    assert controller.settings.overlay.desktop_flet.position.x == 500
    assert controller.settings.overlay.desktop_flet.position.y == 600

    await controller.set_overlay_enabled(False)


@pytest.mark.asyncio
async def test_desktop_move_persistence_ignores_future_epoch_but_allows_current_epoch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _patch_overlay_runtime(monkeypatch)
    saved_desktop: list[tuple[object, object, str]] = []
...
async def test_desktop_interaction_mode_controls_are_desktop_only_and_update_locked_state(
    ...
    assert controller.desktop_overlay_captions_locked is False
    assert controller.settings.overlay.desktop_flet.locked is False
    assert saved_locked[-1] is False
    assert bridge.desktop_runtime_control_payloads[-1] == {
        "command": "set_interaction_mode",
        "mode": "edit",
    }
