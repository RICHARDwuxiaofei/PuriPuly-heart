...
EXPECTED_EULA_BODIES = {
    "ko": (
        "Puri Puly Hearts는 상대방의 음성을 번역하기 위해 데스크탑 오디오를 루프백으로 캡처합니다.\n"
        "이는 타인의 동의 없이, 그리고 자신의 참여하지 않은 대화의 음성이 처리될 수 있음을 의미합니다.\n"
        "이러한 행위는 국가, 지역에 따라 관련 법률에 저촉될 수 있습니다.\n\n"
        "PuriPuly Hearts는 상대방의 오디오나 전사문, 번역 결과를 저장하지 않습니다.\n"
        "다만 설정된 외부 음성 인식, 번역 제공자가 데이터를 처리할 수 있습니다.\n\n"
        "사용에 따른 법적 책임은 사용자에게 있습니다.\n"
        "가능한 한 자신이 참여한 대화에서, 상대방의 동의 하에 사용해 주세요."
    ),
    "zh-CN": (
        "Puri Puly Hearts 为了翻译对方的声音，会通过环回内录截取桌面音频。\n"
        "这就意味着，在没有经过他人同意的情况下，甚至是你自己没有参与的对话，里面的声音也可能会被处理。\n"
        "这种行为根据不同的国家和地区，可能会违反相关法律。\n\n"
        "PuriPuly Hearts 不会存储对方的音频、转写文本或翻译结果。\n"
        "但你配置的外部语音识别和翻译服务商可能会处理这些数据。\n\n"
        "使用时产生的法律责任得由使用者自己承担。\n"
        "所以请尽量在你参与的对话里，并在对方同意的情况下去使用。"
    ),
    "en": (
        "Puri Puly Hearts captures desktop audio via loopback to translate the other person's voice.\n"
        "This means it might process audio from conversations you aren't involved in, without the speakers' consent.\n"
        "Depending on your region, this could violate local laws.\n\n"
        "PuriPuly Hearts does not store the other person's audio, transcripts, or translation results.\n"
...
def test_peer_translation_eula_dialog_renders_single_full_body_key(
    ...
    body_column = modal_content.content.controls[2]
